...
SOURCE_PATH = Path("/Users/karthi/Downloads/vims.vignan.ac.in_SectionAttSubWise.jsp.pdf")
...
def _extract_student_blocks(text: str) -> list[str]:
    text = re.sub(r"https://vims\.vignan\.ac\.in/SectionAttSubWise\.jsp\s+\d+/\d+", "", text)
    text = re.sub(r"\b\d{2}/\d{2}/\d{4},\s+\d{2}:\d{2}\b.*?SectionAttSubWise\.jsp", "", text)
    text = text.split("\nPrint", 1)[0] if "\nPrint" in text else text
...
def _parse_student_attendance(text: str) -> list[dict[str, object]]:
    ...
    for block in _extract_student_blocks(text):
        ...
        overall_values = re.findall(r"(?<![\d(])(\d+\.\d+)(?!%\))", compact)
        ...
        parsed_rows.append(
            {
                ...
                "overall_attendance": float(overall_values[-1]),
...
def import_subject_attendance_section10_pdf(source_path: Path = SOURCE_PATH) -> None:
    ...
    try:
        ...
        db.commit()
        print(f"Stored subject-wise attendance for {updated} students from {source_path.name}.")
    finally:
        db.close()
...
if __name__ == "__main__":
    ...
    import_subject_attendance_section10_pdf(cli_source_path)
